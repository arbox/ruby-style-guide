...
* <a name="indent-when-to-case"></a> Делайте отступ для `when` таким же, как и для `case`.
  Я знаю, что многие не согласятся  с этим, но этот стиль предписывается как "Языком программирования Ruby",
...
* <a name="no-trailing-params-comma"></a>  Избегайте запятых после последнего
  ...
  ```Ruby
  # плохо - легче перемещать/добавлять/удалять строки, 
  # но все же не является предпочтительным
  some_method(
               size,
               count,
               color,
...
* <a name="spaces-around-equals"></a> Вставляйте пробелы вокруг оператора присваивания `=`, когда
  присваиваете значения по умолчанию параметрам метода:<sup>[[ссылка](#spaces-around-equals)]</sup>
...
* <a name="consistent-multi-line-chains"></a> Используйте единый стиль
  ...
  * **(Вариант A)** Когда продолжаете цепочку вызовов методов на
  следующей строке, начинайте её с `.`.
  ...
  * **(Вариант B)** Когда продолжаете цепочку вызовов методов на 
  следующей строке, завершайте строку `.`, чтобы показать, что выражение продолжается
...
* <a name="no-double-indent"></a> Выравнивайте параметры вызова метода, если
  они занимают более одной строки. Если выравнивание невозможно из-за
...
* <a name="no-for-loops"></a> Используйте оператор `for` только в случаях, когда
  вы точно знаете, зачем вы это делаете. В большинстве случаев
...
* <a name="ternary-operator"></a> Предпочитайте тернарный оператор (`?:`)
  конструкциям с `if/then/else/end`. Он используется чаще и очевидно
...
* <a name="use-if-case-returns"></a> Используйте тот факт, что `if` and
...
* <a name="no-bang-bang"></a> Избегайте использования `!!`.
...
* <a name="if-as-a-modifier"></a> Предпочтительнее использовать модификатор `if/unless` для однострочных выражений. Другим хорошим вариантом являются операторы
...
* <a name="no-multiline-if-modifiers"></a>  Избегайте `if/unless` в конце
  ...
  # плохо
  10.times do
    # некоторое многострочное выражение
...
* <a name="while-as-a-modifier"></a> Используйте модификатор `while/until` для однострочный
...
* <a name="no-dsl-parens"></a> Не используйте скобки при вызове методов,
  являющихся частью DSL (таких как Rake, Rails, RSpec), методов, имеющих
...
* <a name="block-argument"></a>Рассмотрите возможность использования блока в аргументе, чтобы избежать написания блочного литерала, который просто передает свои аргументы в другой блок. Остерегайтесь влияния на производительность, хотя, блок преобразуется в Proc.
  ...
  def with_tmp_dir
    Dir.mktmpdir do |tmp_dir|
      Dir.chdir(tmp_dir) { |dir| yield dir }  # блок только передает аргументы
    end
  end
...
* <a name="no-explicit-return"></a> Избегайте `return`, где это не требуется для управления потоком исполнения.<sup>[[ссылка](#no-explicit-return)]</sup>
...
* <a name="no-self-unless-required"></a> Избегайте `self`, где это не требуется. (Требуется только при вызове аксессора для записи.)
